...
if lines.any() != None:
    for line in lines:
        ...
        x2 = int(x0 - 1000*(-b))
        y2 = int(y0 - 1000*a)
        cv2.line(img1_color, (x1,y1), (x2,y2), (0,0,255), 2)

img1_color2 = cv2.cvtColor(img1, cv2.COLOR_GRAY2RGB)
if linesP.any() != None:
    for line in linesP:
        x1, x2, y1, y2 = line[0]
        cv2.line(img1_color2, (x1,x2), (y1,y2), (0,255,0), 2)

circles = np.uint16(np.around(circles))
img2_color = cv2.cvtColor(img2, cv2.COLOR_GRAY2BGR)
if circles.any() != None:
    for i in circles[0, :]:
        cv2.circle(img2_color, (i[0], i[1]), i[2], (0,0,255), 2)

ress = []
ress.append(img1)
ress.append(img2)
ress.append(img1_edge)
ress.append(img1_color)
ress.append(img1_color2)
ress.append(img2_color)
